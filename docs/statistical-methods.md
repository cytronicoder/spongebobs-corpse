...
- Measure of spread in individual measurements
- `SD = sqrt(Σ(xi - mean)² / (n-1))`
- Describes the natural variability in your measurements
- To show the range of individual observations

#### Standard Error (SE)

- Uncertainty in the estimated mean
- `SE = SD / sqrt(n)`
- How precisely we know the true mean value
- To show uncertainty in summary statistics (means)

#### Combined Uncertainty

- Total uncertainty accounting for both random and systematic errors
- `u_combined = sqrt(u_random² + u_instrument²)`
  - `u_random = SE` (statistical uncertainty)
  - `u_instrument` = known instrument uncertainty (time: 0.01 s, force: 0.05 N)
- Best estimate of total measurement uncertainty
- In plots showing experimental results with error bars

#### 95% Confidence Interval (CI)

- Range likely to contain the true population parameter
- `CI = estimate ± t(α/2, df) × SE`
  - `t(α/2, df)` is the critical t-value for 95% confidence
- 95% of CIs computed this way will contain the true value
- For regression parameters and pairwise comparisons
...
- Slope uncertainty
  ...
  - If the CI excludes zero, the slope is significantly different from zero
...
- `CI(mean) = ŷ ± t × RMSE × sqrt(1/n + (x - mean_x)²/Σ(xi - mean_x)²)`
